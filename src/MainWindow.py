...
from PyQt5.QtWidgets import (QLabel, QProgressBar, QStyle, QWidget, QVBoxLayout, QApplication, 
                             QGridLayout, QStatusBar, QTextEdit, QMessageBox, QLCDNumber, QGroupBox)
from PyQt5.QtCore import QObject, QStateMachine, QTimer,  pyqtSignal, pyqtSlot, QThread, QState, Qt


from CalibrationWidget import CalibrationWidget
from CommandButton import  Command
from PromptWidget import PromptWidget
from SerialConnectionWidget import SerialConnectionWidget
from SerialInterface import SerialInterface
from WeighAndCountWidget import WeighAndCountWidget
...
class Response(QObject):
    ...
    def Count(self, cmd:Command) -> None:
        text = f"There are {cmd.returnValue} items, in the basket.\n\n{cmd.promptProceed}"
        self.prompt.emit(text)
        self.weigh_and_count.emit(cmd)
...
class MainWindow(QWidget):
    ...
    def setupUi(self):
        ...
        self.gridLayout.addWidget(self.GBox_Led, 0*ROW, 0*COL, 1*HEIGHT, 2*WIDTH)
        self.gridLayout.addWidget(self.prompt, 1*ROW, 0*COL, 3*HEIGHT, 1*WIDTH)
        self.gridLayout.addWidget(self.serial_connection, 1*ROW, 1*COL, 1*HEIGHT, 1*WIDTH)
        self.gridLayout.addWidget(self.weigh_and_count, 2*ROW, 1*COL, 1*HEIGHT, 1*WIDTH)
        self.gridLayout.addWidget(self.calibration, 3*ROW, 1*COL, 1*HEIGHT, 1*WIDTH)
        self.gridLayout.addWidget(self.GBox_ProgressBar, 4*ROW, 0*COL, 1*HEIGHT, 2*WIDTH)
    ...
    def runConnections(self):
        ...
        self.worker.serial_disconnected.connect(self.serial_connection.response.disconnected)
        ...
        self.worker.serial_receive.connect(self.prompt.serial_data_viewer.ViewDataReceived)
        self.worker.serial_status.connect(self.prompt.serial_data_viewer.ViewSerialStatus)
        ...
        self.serial_connection.request.prompt.connect(self.prompt.TextEdit_Prompt.setText)
        self.worker.live_data.connect(self.LCDLiveData)
        
        self.request.reset_progresscounter.connect(self.ResetProgressBar)
        self.dataviewer_liveupdate.connect(self.prompt.serial_data_viewer.live_plot_update)
    ...
    def setupStates(self):
        ...
        # disconnected        
        self.state_disconnected.addTransition(self.serial_connection.response.connected, self.state_home)
        self.state_disconnected.addTransition(self.autoconnect_success, self.state_home)
                
        # home ---> 
        # home ---> calibration
        self.state_home.addTransition(self.calibration.Tare_Button.clicked, self.state_calibration)
        self.state_home.addTransition(self.calibration.Calibration_Button.clicked, self.state_calibration)
        # home ---> weigh and count
        self.state_home.addTransition(self.weigh_and_count.response.initiated, self.state_weighandcount)
        # home ---> disconnected
        self.state_home.addTransition(self.serial_connection.response.disconnected, self.state_disconnected)
        
        
        # calibration ---> home
        self.state_calibration.addTransition(self.calibration.response.calibration_complete, self.state_home)
        self.state_calibration.addTransition(self.serial_connection.response.disconnected, self.state_disconnected)
        
        # weigh and count ---> home
        self.state_weighandcount.addTransition(self.weigh_and_count.response.weigh_and_count_finished, self.state_home)
        self.state_weighandcount.addTransition(self.serial_connection.response.disconnected, self.state_disconnected)
        self.state_weighandcount.addTransition(self.weigh_and_count.response.reset_state, self.state_home)
    ...
    def LCDLiveData(self, serIn):
        try:
            ...
            if rawint > 950:
                ...
                self.prompt.TextEdit_Prompt.setText("ERROR: Too heavy! Maximum Wieght Reached!")
            else:
