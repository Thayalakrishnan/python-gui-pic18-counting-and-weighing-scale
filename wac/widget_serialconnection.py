...
from wac.command_button import Command, MultiCommandButton
from wac.router import Router
from wac.command_button import SerialCommands
...
class SerialConnectionWidget(QWidget):
    ...
    def setupStates(self):
        ...
        self.state_disconnected.addTransition(
            self.response.connected, self.state_connected
        )
        self.state_connected.addTransition(
            self.response.disconnected, self.state_disconnected
        )
        self.state_disconnected.addTransition(
            self.response.autoconnected, self.state_connected
        )
